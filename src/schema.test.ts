...
describe("AEAT XSD validation", () => {
  it("accepts the generated RegistroAlta", () => {
    expect(validateVerifactuXsd(buildVerifactuXml(input, "A".repeat(64), software))).toEqual({ valid: true });
  });
  ...
  it("accepts a rectificative RegistroAlta with its references", () => {
    const xml = buildVerifactuXml({
      ...
      invoiceType: "R1",
      rectificationType: "S",
      rectifiedInvoices: [{ invoiceNumber: "A-000000", issueDate: new Date("2026-03-01T00:00:00.000Z") }],
      rectificationAmounts: { base: 90, tax: 18 },
    }, "A".repeat(64), software);
