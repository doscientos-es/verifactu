...
describe("durable VERI*FACTU engine", () => {
	...
	it("verifies the stored Alta hash before preparing delivery", () => {
		...
		expect(() => prepareDurableVerifactuRecord({
			recordType: "alta",
			payload: altaPayload,
			currentHash: "A".repeat(64),
		}, software)).toThrow("La huella del ledger no coincide");
	});
	...
	it("decodes and verifies cancellation payloads", () => {
		...
		}, software).recordType).toBe("anulacion");
	});
	...
	it("fails closed for malformed snapshots and cancellation markers", () => {
		...
		expect(() => parseDurableCancellationPayload({
			nif: "B12345678",
	...
	it("centralizes retry policy, wait time, error format and response minimization", () => {
		...
		expect(formatVerifactuDeliveryError(null, result)).toBe(
			"AEAT 4102: Servicio no disponible",
		);
