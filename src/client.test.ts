...
describe('verifactu/client', () => {
  ...
  it('buildVerifactuXml includes all mandatory fiscal fields', () => {
    ...
    expect(xml).toContain('<sf:NombreRazonEmisor>Test Company S.L.</sf:NombreRazonEmisor>')
    expect(xml).toContain('<sf:NumSerieFactura>A-000001</sf:NumSerieFactura>')
    expect(xml).toContain('<sf:FechaExpedicionFactura>15-03-2026</sf:FechaExpedicionFactura>')
    expect(xml).toContain('<sf:TipoFactura>F1</sf:TipoFactura>')
    expect(xml).toContain('<sf:DescripcionOperacion>Servicios de prueba</sf:DescripcionOperacion>')
    ...
    expect(xml).toContain('<sf:IdSistemaInformatico>D1</sf:IdSistemaInformatico>')
  })
  ...
  it('buildVerifactuXml chains via RegistroAnterior when previousHash + prev invoice ID are set', () => {
    ...
    expect(xml).toContain('<sf:NumSerieFactura>A-000001</sf:NumSerieFactura>')
    expect(xml).toContain('<sf:FechaExpedicionFactura>15-01-2026</sf:FechaExpedicionFactura>')
  ...
  it('buildVerifactuXml falls back to PrimerRegistro when previousInvoiceNumber is missing', () => {
    ...
    const xml = buildVerifactuXml({ ...baseInput, previousHash: 'abc123' }, 'newhash', mockSoftware)
    expect(xml).toContain('<sf:PrimerRegistro>S</sf:PrimerRegistro>')
  })
