      ...
      (response) => {
        ...
        response.on('data', (chunk: Buffer) => {
          ...
          if (size > maxResponseBytes)
            request.destroy(new Error('La respuesta de AEAT excede el límite'))
          else chunks.push(chunk)
        })
        response.on('end', () => {
          if ((response.statusCode ?? 500) >= 400)
            reject(new Error(`AEAT respondió HTTP ${response.statusCode}`))
          else resolve(Buffer.concat(chunks).toString('utf8'))
        })
      },
    )
    request.once('timeout', () =>
      request.destroy(new Error('Tiempo de espera agotado al consultar AEAT')),
    )
