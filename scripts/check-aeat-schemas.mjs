...
for (const schema of schemas) {
  ...
  const local = normalise(await readFile(fileURLToPath(new URL(`../src/schemas/${schema.name}`, import.meta.url)), "utf8"));
...
if (changed) {
  console.error("Los XSD locales difieren de los publicados por AEAT. Revisar cambios antes de desplegar.");
  process.exitCode = 1;
}
